...
if [[ $TRAVIS_BRANCH == "master" ]]; then
	...
	git pull "https://git:${GH_TOKEN}@github.com/IBM/@onnetsystems-wat/onnet-design-systems.git" gh-pages
	...
	if [ $? -eq 0 ]; then
		git push --force "https://git:${GH_TOKEN}@github.com/IBM/@onnetsystems-wat/onnet-design-systems.git" master:gh-pages > /dev/null 2>&1
	fi
fi
